...
export class EntityService {
  ...
  private readonly apiFn: {
    ...
    ): Promise<(axios?: AxiosInstance, basePath?: string) => AxiosPromise<Entity>>;
  ...
  getEntities(vocabID: string, text?: string, tagType?: Array<TagType>, page = 0, rowCount = 10): Promise<EntityList | undefined> {
    return this.apiFn.listEntities(vocabID, text, tagType, undefined, undefined, undefined, page, rowCount).then((req) =>
      req().then((resp) => {
  ...
  getEntity(vocabID: string, entityID: string): Promise<Entity | undefined> {
    return this.apiFn.vocabVocabularyIdEntitiesEntityIdGet(vocabID, entityID).then((req) =>
      req().then((resp) => {
        if (this.noStatusError(resp.status, resp)) {
          return resp.data;
        }
  ...
  updateEntity(entity: Entity): Promise<Entity | undefined> {
    ...
    return this.apiFn.vocabVocabularyIdEntitiesEntityIdPut(entity.vocabulary, entity.id, lastModified, entity).then((req) =>
      req().then((resp) => {
        if (this.noStatusError(resp.status, resp)) {
          return resp.data;
        }
  ...
  deleteEntity(entity: Entity): Promise<Entity | undefined> {
    ...
    return this.apiFn.vocabVocabularyIdEntitiesEntityIdDelete(entity.vocabulary, entity.id, lastModified).then((req) =>
      req().then((resp) => {
        if (this.noStatusError(resp.status, resp)) {
          return resp.data;
        }
