...
const arrayToMap = (array: Vocabulary[]): Map<string, Vocabulary> => {
  return new Map<string, Vocabulary>(array.map((task: Vocabulary) => [task.id, task]));
};
...
export const vocabStore: Module<any, any> = {
  ...
  mutations: {
    LOADVOCABLIST(state, payload: {searchText?: string; page?: number; rowCount?: number}) {
      vocabService.getVocabs(payload.searchText, payload.page, payload.rowCount).then((list: VocabList | undefined) => {
    ...
    LOADNEXTVOCABPAGE(state) {
      ...
      if (state.pageable.totalItems > 0 && nextPage < state.pageable.totalItems) {
        ...
        vocabService.getVocabs(searchText, nextPage, nextRowCount).then((list) => {
          if (list && list.vocabs.length > 0) {
            state.vocabularies = new Map<string, Vocabulary>(...state.vocabularies, arrayToMap(list.vocabs));
            state.pageable = list.pageable;
          }
        });
      } else {
    ...
    CREATEVOCAB(state, payload: {label?: string; description?: string}) {
      vocabService.createVocab(payload.label, payload.description).then((vocab) => {
        if (vocab) {
          state.vocabularies.set(vocab.id, vocab);
        }
      });
    ...
    UPDATEVOCAB(state, payload: {label?: string; description?: string}) {
      ...
      vocabService.updateVocab(state.vocabulary).then((vocab: Vocabulary | undefined) => {
        if (vocab) {
          state.vocabularies.set(vocab.id, vocab);
        }
      });
    ...
    DELETEVOCAB(state, payload: {vocabulary: Vocabulary}) {
      vocabService.deleteVocab(payload.vocabulary).then((vocab: Vocabulary | undefined) => {
        if (vocab) {
  ...
  getters: {
    vocabularies: (state) =>
      Array.from<Vocabulary>(state.vocabularies.values()).sort(
        (t1: Vocabulary, t2: Vocabulary) => Date.parse(t1.created) - Date.parse(t2.created)
      ),
